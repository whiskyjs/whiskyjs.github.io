...
Итог анализа: мы совершенно точно не можем внедрять зависимости в конструктор класса компонента
без модификации ядра.

Но еще раз перечитаем ТЗ и посмотрим на код свежим взглядом - вопрос-то не в этом.
Не в технической реализации абстрактного паттерна ради того, чтобы она была,
а чтобы наш `FooComponent` получал в конструктор экземпляры зависимостей.
